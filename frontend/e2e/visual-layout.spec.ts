...
async function writeScreenshot(
  ...
  const artifactRoot =
    process.env.QA_ARTIFACT_ROOT?.trim() ||
    resolve(process.cwd(), "..", "tmp", "qa-runs");
  const runId =
    process.env.QA_RUN_ID ||
    `${new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19)}-visual-layout`;
...
async function assertVisualCase(
  ...
  await expect(
    page.getByRole("heading", { name: visualCase.heading }),
  ).toBeVisible();
  ...
  expect(
    layoutViolations,
    `${visualCase.path} layout violations at ${viewport}px`,
  ).toEqual([]);
  expect(
    tracker.issues.pageErrors,
    `${visualCase.path} page errors at ${viewport}px`,
  ).toEqual([]);
  expect(
    tracker.issues.consoleErrors,
    `${visualCase.path} console errors at ${viewport}px`,
  ).toEqual([]);
}
