            ...
            <span class="min-w-0 flex-1">
              ...
              <span class="mt-2 flex items-center justify-between gap-3">
                <span dir="auto" class:font-bold={conversation.unread_count > 0} class="truncate text-xs text-slate-500">
                  {conversation.last_message?.body || (conversation.last_message?.document ? conversation.last_message.document.filename : conversation.last_message?.message_type === 'voice_note' ? $_('messaging.voiceNote') : conversation.last_message?.photo_count ? $_('messaging.photoMessage') : $_('messaging.noMessagesYet'))}
                </span>
