...
<section class="reports-page mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
  ...
  {:else}
    ...
    {:else if overview && breakdowns}
      ...
      <section class="report-card mt-7 card p-5">
      ...
      <div class="mt-7 grid gap-6 lg:grid-cols-2">
        {@render SummaryTable($_('reports.classComparison'), breakdowns.classes, { key: 'classSectionId', label: $_('reports.classSection') })}
        {@render SummaryTable($_('reports.gradeComparison'), breakdowns.grades, { key: 'gradeLevelId', label: $_('reports.grade') })}
        {@render SummaryTable($_('reports.subjectComparison'), breakdowns.subjects, { key: 'subjectId', label: $_('reports.subject') })}
        {@render SummaryTable($_('reports.dutyHotspots'), breakdowns.duty_contexts, { key: 'dutyContext', label: $_('reports.dutyContext') }, true)}
        {@render SummaryTable($_('reports.categoryBreakdown'), breakdowns.categories, { key: 'categoryId', label: $_('reports.category') })}
      </div>
      ...
      <section class="report-card mt-7 card p-5" bind:this={eventsSection} tabindex="-1">
        <div class="flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="flex gap-2">
            ...
            <button onclick={() => { eventsOpen = !eventsOpen; if (eventsOpen) void loadEvents(0); }} class="rounded-xl border border-slate-300 px-4 py-2 text-sm font-bold">{eventsOpen ? $_('reports.hide') : $_('reports.openDrilldown')}</button>
          </div>
        </div>
