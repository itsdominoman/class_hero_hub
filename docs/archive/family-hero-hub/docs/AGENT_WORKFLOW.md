...
- **Mandatory Docs Sync:** Whenever a code change affects something documented, update ALL relevant docs in the same commit or a closely-following doc commit — not only `PLAN.md`'s changelog, but also `DESIGN.md`, `LOCALISATION_NOTES.md`, `ROADMAP.md`, `PROJECT_STATUS.md`, `QA_COVERAGE_MATRIX.md`, the README, or any other file under `docs/` whose content the change makes outdated or incomplete. If unsure whether a doc is affected, open it and check.
- **Backend deploys need a container rebuild:** the backend image bakes code at build time (`build: ./backend`, only `./data` mounted). After any backend change run `docker compose build backend && docker compose up -d backend` before dev testing; a frontend-only rebuild leaves the API on stale code.
