...
- Removing a school item from the weekly list **soft-deletes** it (`is_active = False`), it does not hard-delete (FIX 1). A packed item is referenced by a `school_item_checks` row via an ON DELETE NO ACTION FK, so hard-deleting one that had ever been packed previously raised a PostgreSQL FK violation. Soft-delete drops it from the current list while preserving packing history; every read path already filters `is_active`. Covered by a real-PostgreSQL FK-enforcement test (`tests/test_school_items_fk_pg.py`) plus a SQLite behavioural test.
