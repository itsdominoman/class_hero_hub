...
- **Parent mark-packed + time-windowed summary (D1/D2/D3) Coverage:** `test_school_items.py` adds, at both layers: **D3 pure window/tile functions** (`needed_today_window_open` open at every hour; `pack_tomorrow_window_open` False at 17:59, True from 18:00; `compute_tile_state` returning `missing_today` in the morning when items remain, `none` when resolved before 6pm, `pack_tomorrow` in the evening, and `missing_today` taking priority when both apply) tested directly with fixed datetimes since they take `now_local` as an argument. **D2 parent override:** service-level `set_packed(..., enforce_lock=False)` packs a locked today item that the child path rejects (`checklist_locked`), and HTTP `POST /api/school-items/{id}/pack` (parent auth) marks today packed (`locked:true` echoed, shared row written, idempotent) and returns `404` for a child outside the parent's family. **D3 summary:** HTTP `GET /api/school-items/summary` returns `configured`, the window flags (the `pack_tomorrow` flag derived from the real family clock at test time), a child with an unpacked today item in `needed_today` with `tile_mode:"missing_today"`, then — after the parent resolves it — that child dropping out (`needed_today: []`, `tile_mode:"none"`); plus an unconfigured-family empty response. **D1 is frontend-only** (modal no longer falls through to the points form): confirmed by `svelte-check`/build, no backend surface. Frontend (manual/visual): the "Mark packed" button is an optimistic-refresh mutation surface outside the read-only harness; the tile hides outside any in-window/actionable state.
- **Today calendar tile + parent task completion (E1/E2) Coverage:** `test_calendar_logic.py` adds, at both layers: **E1 pure rules** — `task_is_outstanding` (events never outstanding; a task outstanding for `None`/`pending`/`rejected`, done only when `approved`) and `count_attention_items` (events + outstanding tasks, e.g. 5 of a 6-item mix, 0 for empty). **E1 summary endpoint** — `GET /api/calendar/summary` returns the unconfigured empty shape (`{configured:false, tile_count:0, today:[], tomorrow:[]}`) before any entry; after creating today's event + two today tasks (one approved via the complete route) + a tomorrow event, `configured:true`, `tile_count:2` (event + one open task; approved task excluded), the child present in both `today` (all 3 items listed) and `tomorrow` (1 item); and a subsequently-added **pending** child-claim leaves `tile_count` unchanged (still outstanding). **E2 is no new backend** — the parent-complete route (`POST /api/calendar/{id}/complete`) was already covered for finality (`status="approved"`) and streak-multiplier award by `test_streak_bonus_multiplier` / `test_duplicate_completion_blocked`. **Latent fix** — adding `from_attributes` to the `CalendarCompletion` schema is exercised by the summary test (which serializes ORM completions). Frontend (manual/visual): the "Mark complete" button is an optimistic-refresh mutation surface; a child-claimed (pending) task shows a read-only "Awaiting your review" pill (no complete button), keeping E1 distinct from the C10 review card.
- **Today modal "Coming up tomorrow" + full-calendar link (F1/F2) Coverage:** no new backend — `/calendar/summary` already returns full tomorrow occurrences (with titles), so F1 is a frontend re-render of existing data and F2 is a static link. Frontend (manual/visual): the look-ahead lists tomorrow's event titles per child (tasks dropped), children with no tomorrow event fall out, and "Nothing on the calendar tomorrow" shows when none have events; the footer "Open full calendar" link routes to `/calendar`. The existing summary-endpoint test already asserts the tomorrow occurrence (title carried on `entry`) is present, covering the data F1 renders.
