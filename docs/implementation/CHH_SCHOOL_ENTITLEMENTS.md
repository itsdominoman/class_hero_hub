...
The platform manager shows each capability relationship in both directions: a child states what it requires and a parent states which features use it. An invalid enable, disable or availability-date change opens a plain-language English or Arabic explanation and links directly to the affected capability cards. The manager never switches off related capabilities automatically; the existing server validation remains the final authority if the page is stale or another session changes the same school.
