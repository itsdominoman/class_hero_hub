...
| 5 | Staff-to-staff messaging | Same-school active-staff discovery and messaging are deployed. The user's basic Teacher Messages check passed. Read-only live reconciliation found two active staff-direct conversations, five active messages, 17 delivered and 18 read receipt events; two staff-direct notification events were provider-accepted and one was correctly cancelled because the recipient had no active device. The required attachment, disabled-account, cross-school and mobile cases were not manually accepted. | Execute the remaining bounded staff-message matrix with two staff identities and one disabled/cross-school negative case. |
